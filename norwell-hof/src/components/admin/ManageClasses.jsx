...
import { createClass, getAllClasses, updateInductee } from '../../firebase/firestore'; // Note: updateInductee should likely be updateClass
import { Plus, Calendar, Trash2, Edit } from 'lucide-react';

// Import the utility function from the separate file
import { deleteInductionClass } from './HandleDelete.jsx'; 
...
const ManageClasses = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (editingId) {
            // Update existing class (Assuming you have an updateClass utility)
            // Note: I'm using updateInductee here as per your original file, but updateClass is better practice.
    ...
    /**
     * Handles the deletion of an Induction Class document.
     * Calls the deleteInductionClass utility function.
     */
    const handleDelete = async (classItem) => {
        // 1. Confirmation Dialog
        const confirmDelete = window.confirm(
            `Are you sure you want to delete the Class of ${classItem.year}? \n\nThis action cannot be undone and will permanently remove this class entry.`
        );
        ...
        try {
            // 2. Call the deletion utility function
            const { error } = await deleteInductionClass(classItem.id);
            ...
            setMessage({ type: 'success', text: `Class of ${classItem.year} deleted successfully.` });
            
            // Remove the deleted class from the local state list
            setClasses(prevClasses => prevClasses.filter(i => i.id !== classItem.id));
            
            // If the deleted class was being edited, clear the form
            ...
            setMessage({ type: 'error', text: `Failed to delete class: ${error.message}` });
        }
