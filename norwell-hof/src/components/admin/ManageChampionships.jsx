...
import { deleteChampionshipWithPhoto } from './HandleDelete.jsx'; 
...
const ManageChampionships = () => {
    ...
    const handleDelete = async (championship) => {
        const confirmDelete = window.confirm(
            `Are you sure you want to delete the championship: ${championship.title} (${championship.year})? \n\nThis action will also permanently delete the associated photo from storage.`
        );
        ...
        try {
            // Call the utility function to delete the document AND the photo
            const { error } = await deleteChampionshipWithPhoto(championship);

            if (error) {
                throw new Error(error);
            }

            setMessage({ type: 'success', text: `Championship '${championship.title}' deleted successfully!` });
            
            // Update local state to remove the deleted item
            setChampionships(prev => prev.filter(c => c.id !== championship.id));

            // Clear editing state if the deleted item was being edited
            if (editingId === championship.id) {
                handleCancelEdit();
            }

        } catch (error) {
            console.error('Error deleting championship:', error);
            setMessage({ type: 'error', text: `Failed to delete championship: ${error.message}` });
        }
        setLoading(false);
    };
    // 🔚 END OF UPDATED DELETE HANDLER
                        ...
                        {filteredChampionships.map((championship) => (
                            ...
                            >
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button 
                                        onClick={() => handleDelete(championship)} // Pass the full object, not just the ID
